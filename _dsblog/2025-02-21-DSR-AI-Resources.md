    ...
    {% if section != "" %}
      ...
      {% if category_name != "" and category_name != "No Resources Available" %}
        <details class="category">
          ...
          <div class="content">
            <ol>
              {% assign content_lines = lines.last | split: "\n" %}
              {% for line in content_lines %}
                {% assign stripped_line = line | strip %}
                {% if stripped_line != "" and stripped_line contains "1. " %}
                  <li>
                    {% if stripped_line contains "[" and stripped_line contains "](" %}
                      ...
                      {% assign link_url_with_extra = link_parts.last | strip %}
                      {% assign link_url_temp = link_url_with_extra | split: ")" | first | strip %}
                      {% assign extra_info = link_url_with_extra | split: "} :" | last | strip %}
                      ...
                      {% if extra_info != "" and extra_info != link_url_temp %}
                        {{ extra_info }}
                      {% endif %}

                    {% elsif stripped_line contains "http" %}
                      <!-- Handle plain URLs -->
                      {% assign url_and_info = stripped_line | split: ": " %}
                      {% assign plain_url = url_and_info.first | remove: "1. " | remove: "- " | strip %}
                      {% assign extra_info = url_and_info.last | strip %}
                    ...
                    {% else %}
                      <!-- Handle plain text -->
                      {% if stripped_line contains "1. " %}
                        {{ stripped_line | remove: "1. " | strip | escape }}
                      {% else %}
                        {{ stripped_line | escape }}
                      {% endif %}
                    {% endif %}
                  </li>
                {% elsif stripped_line != "" and stripped_line contains "##" %}
                  <!-- Handle subheadings -->
                  <h3>{{ stripped_line | remove: "## " | strip | escape }}</h3>
                {% endif %}
              {% endfor %}
            </ol>
          </div>
